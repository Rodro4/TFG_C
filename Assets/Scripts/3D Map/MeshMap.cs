...
public class MeshMap : MonoBehaviour
{
    ...
    public bool generateMesh = true;
    public float meshUpdateInterval = 1.0f;
    // Cuántos cubos se añaden al mesh por frame durante el rebuild incremental.
    // Sube si tienes CPU de sobra, baja si sigues teniendo spikes.
    public int rebuildChunkSize = 500;
    public int maxVoxels = 50000;
    ...
    public struct VoxelPoint { public Vector3 pos; public Color32 col; }
    ...
    private float timer = 0f;
    private bool needsColliderUpdate = false;
    private bool rebuildInProgress = false;  // evita solapar rebuilds
    ...
    void Awake()
    {
        ...
        var mr = GetComponent<MeshRenderer>();
        if (voxelMaterial != null) mr.material = voxelMaterial;
        else Debug.LogWarning("MeshMap: Asigna voxelMaterial.");
    ...
    void Start()
    {
        if (useROS)
        {
            ...
            else { useROS = false; Debug.LogWarning("MeshMap: RosConnector no encontrado."); }
        }
        initialized = true;
        OnEnable();
    }
    ...
    public List<VoxelPoint> GetPoints()
    {
        lock (voxelLock)
        {
            cached.Clear();
            cached.Capacity = voxels.Count;
            foreach (var v in voxels.Values) cached.Add(v);
            return cached;
        }
    }
    ...
    public void ClearAll()
    {
        lock (voxelLock) { voxels.Clear(); cached.Clear(); }
        mesh.Clear();
    ...
    void Update()
    {
        ...
        if (!generateMesh || !dirty || rebuildInProgress) return;
        ...
        // Rebuild distribuido en frames para evitar spike
        StartCoroutine(RebuildMeshCoroutine());
    }

    // ─── MESH BUILD (distribuido) ────────────────────────────────────
    IEnumerator RebuildMeshCoroutine()
    {
        rebuildInProgress = true;

        // Snapshot thread-safe de los voxels actuales
        List<VoxelPoint> pts;
        lock (voxelLock)
        {
            pts = new List<VoxelPoint>(voxels.Count);
            foreach (var v in voxels.Values) pts.Add(v);
        }

        var verts = new List<Vector3>(pts.Count * 8);
        var cols  = new List<Color32>(pts.Count * 8);
        var tris  = new List<int>(pts.Count * 36);

        int index = 0;
        for (int i = 0; i < pts.Count; i++)
        {
            AddCube(pts[i].pos, pts[i].col, index, verts, cols, tris);
            index += 8;

            // Ceder el control cada rebuildChunkSize cubos para no bloquear el frame
            if (i % rebuildChunkSize == 0)
                yield return null;
        }

        // Aplicar al mesh en el main thread (ya estamos en él, coroutine)
        mesh.Clear();
        mesh.SetVertices(verts);
        mesh.SetColors(cols);
        mesh.SetTriangles(tris, 0);
    ...
    {
        float s = voxelSize * 0.5f;
        verts.Add(p + new Vector3(-s, -s, -s)); verts.Add(p + new Vector3( s, -s, -s));
        verts.Add(p + new Vector3( s,  s, -s)); verts.Add(p + new Vector3(-s,  s, -s));
        verts.Add(p + new Vector3(-s, -s,  s)); verts.Add(p + new Vector3( s, -s,  s));
        verts.Add(p + new Vector3( s,  s,  s)); verts.Add(p + new Vector3(-s,  s,  s));

        for (int j = 0; j < 8; j++) cols.Add(c);

        int[] t = { 0,2,1, 0,3,2, 1,2,6, 6,5,1, 4,5,6, 6,7,4,
                    2,3,7, 7,6,2, 0,7,3, 0,4,7, 0,1,5, 0,5,4 };
        foreach (var tri in t) tris.Add(i + tri);
    }
}